  ...
  <div class="max-w-3xl mx-auto px-4 py-8">
    ...
    <form @submit.prevent="handleSubmit" class="bg-gray-900 rounded-xl border border-gray-800 p-6 flex flex-col gap-5">
      ...
      <div v-if="isEdit" class="flex items-center gap-2">
        ...
        <label for="is_active" class="text-sm text-gray-300 cursor-pointer">Juego activo (visible en catálogo)</label>
      </div>
...
async function handleSubmit() {
  ...
  try {
    ...
    if (form.value.metacritic_score) payload.metacritic_score = parseInt(form.value.metacritic_score)
